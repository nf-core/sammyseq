process MERGE_COMPARTMENTS {
    ...
    output:
    tuple val(sample_id), path("${sample_id}_merged_compartments.bed"), emit: merged_beds
    tuple val(sample_id), path("${sample_id}_merged_compartments_eigen.bedgraph"), emit: merged_bedgraphs
