...
When the `--differential_solubility` parameter is enabled, the pipeline performs differential solubility analysis between specified sample groups. This analysis compares accessible fractions (e.g., S2S) with less accessible fractions (e.g., S3) using the ratios specified in the `--comparison` parameter to identify genomic regions with significantly different solubility patterns between experimental conditions.

- `--compare_groups`: Specifies which sample groups to compare (format: "GroupAvsGroupB" or "GroupAvsGroupB,GroupAvsGroupC")
- Sample groups must be defined in the `sample_group` column of the input samplesheet
- `--comparison`: The ratios used for analysis (e.g., "S2SvsS3")
- `--keep_regions_bed`: Genomic regions to include in the analysis

The genome is partitioned into bins of equal size (default: `--binsize 50000`) within the regions specified by `--keep_regions_bed`. For each bin, the pipeline calculates normalized signal values and performs statistical comparisons between the specified sample groups using the selected comparison ratios.
