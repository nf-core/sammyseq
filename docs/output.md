...
Consensus profiles are then computed by merging compartment calls from all replicates within the same sample group. For each genomic bin defined according to the resolution set by the `--binsize` parameter, the consensus label is assigned based on the majority vote among replicates. A bin is labeled A or B depending on whether it receives a majority of calls for that compartment; otherwise, an NA value is assigned. The input for this step is the set of combined compartment BED files, and the consensus output is sorted and formatted for IGV visualization.
