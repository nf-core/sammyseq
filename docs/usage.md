...
The minimum reference genome requirements is the FASTA file, provided with the mandatory parameter `--fasta`, the aligner index will be generated by the pipeline and can be saved for later reuse if the `--save_reference` parameter is passed. The index building step can be quite a time-consuming process and it permits their reuse for future runs of the pipeline to save disk space, if already present it can be passed using the `--bwa_index '/path/to/bwa/index/'` or `--bowtie2_index '/path/to/bowtie2/index/'` parameter.
